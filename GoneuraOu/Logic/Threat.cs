    ...
    public static class Threat
    {
        public static bool IsAttacked(this Board.Board board, int square, int turn)
        {
            ...
            return
                ...
                || (Attacks.GoldAttacks[turn ^ 1, square] & board.Bitboards[(int)Piece.SenteGold + tt]) != 0
                || (Attacks.GoldAttacks[turn ^ 1, square] & board.Bitboards[(int)Piece.SenteTokin + tt]) != 0
                || (Attacks.GoldAttacks[turn ^ 1, square] & board.Bitboards[(int)Piece.SentePromotedSilver + tt]) != 0
                ...
                || (Attacks.GetRookAttacks(square, board.Occupancies[2]) &
                    board.Bitboards[(int)Piece.SenteRook + tt]) != 0
                || (Attacks.GetBishopAttacks(square, board.Occupancies[2]) &
                    board.Bitboards[(int)Piece.SenteBishop + tt]) != 0
                || (Attacks.KingAttacks[square] & board.Bitboards[(int)Piece.SenteKing + tt]) != 0
                || (Attacks.GetDragonAttacks(square, board.Occupancies[2]) &
                    board.Bitboards[(int)Piece.SenteDragon + tt]) != 0
